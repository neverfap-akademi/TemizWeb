...
UPSTREAMS={
 ...
 'domains':'https://raw.githubusercontent.com/hagezi/dns-blocklists/main/domains/nsfw.txt',
}
...
def main():
    ...
    if a.fixture_dir:
        ...
        adblock=fetch(UPSTREAMS['adblock']); domain_text=fetch(UPSTREAMS['domains']); minimum=1000

    external_rules=[]
    for s in adblock.splitlines():
        s=s.strip()
        if s.startswith('||') and s.endswith('^') and '$' not in s: external_rules.append(s)
    if len(external_rules)<minimum: raise RuntimeError('Too few upstream adblock rules')
    adult={d for line in domain_text.splitlines() if (d:=normalize_domain(line))}
    if len(adult)<minimum: raise RuntimeError('Too few upstream domains')
